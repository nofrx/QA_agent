...
def test_clean_model_passes():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(file_mb=3.0), "autoshadow": _mock_geom(file_mb=3.5)}
    report = analyze(geom, {})
...
def test_flipped_normals_fails():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(flipped=18), "autoshadow": _mock_geom()}
    report = analyze(geom, {})
...
def test_raw_vs_touchedup_basecolor_is_expected():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(), "autoshadow": _mock_geom()}
    diffs = {"basecolor": {"raw_vs_touchedup": _mock_diff(pct=90.0)}}
    report = analyze(geom, diffs)
    expected = [f for f in report.findings if f.severity == "expected" and "basecolor" in f.title.lower()]
    assert len(expected) >= 1
    assert "artist" in expected[0].explanation.lower()


def test_autoshadow_normal_change_warns():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(), "autoshadow": _mock_geom()}
    diffs = {"normal": {"touchedup_vs_autoshadow": _mock_diff(pct=15.0, max_diff=90)}}
    report = analyze(geom, diffs)
    assert report.verdict == "NEEDS_REVIEW"
    warnings = [f for f in report.findings if f.severity == "warning"]
    assert any("normal" in f.title.lower() and "autoshadow" in f.title.lower() for f in warnings)


def test_autoshadow_normal_minimal_is_ok():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(), "autoshadow": _mock_geom()}
    diffs = {"normal": {"touchedup_vs_autoshadow": _mock_diff(pct=0.5, max_diff=10)}}
    report = analyze(geom, diffs)
    expected = [f for f in report.findings if f.rule_id == "tex_autoshadow_normal_ok"]
    assert len(expected) == 1


def test_doubled_normal_is_critical():
    geom = {"raw": _mock_geom(), "touchedup": _mock_geom(), "autoshadow": _mock_geom()}
    diffs = {"normal": {"raw_vs_touchedup": _mock_diff(pct=80.0, max_diff=255, mean_diff=120.0)}}
    report = analyze(geom, diffs)
    critical = [f for f in report.findings if f.severity == "critical"]
    assert any("doubled" in f.title.lower() for f in critical)
